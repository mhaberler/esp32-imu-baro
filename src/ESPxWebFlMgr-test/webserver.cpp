...
// WARNING: This webserver has no access control. It servers everything from
// your ESP32 file system. WARNING: This webserver has no access control. It
// servers everything from your ESP32 file system.
...
void genericWebServerHandleNotFound(WebServer *webserver) {
  ...
  for (uint8_t i = 0; i < webserver->headers(); i++) {
    message +=
        " " + webserver->headerName(i) + ": " + webserver->header(i) + "\n";
  }
...
void genericWebserverServerNotFound(WebServer *webserver) {
  ...
  // (What is more expensive? Checking SD or header first?)
