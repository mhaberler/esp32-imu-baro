...
// https://github.com/mhaberler/ESP-File-Server/blob/main/main.cpp

/*
  This software, the ideas and concepts is Copyright (c) David Bird 2021
  All rights to this software are reserved.
  It is prohibited to redistribute or reproduce of any part or all of the
  software contents in any form other than the following:
  1. You may print or download to a local hard disk extracts for your personal
  and non-commercial use only.
  2. You may copy the content to individual third parties for their personal
  use, but only if you acknowledge the author David Bird as the source of the
  material.
  3. You may not, except with my express written permission, distribute or
  ...
  5. You MUST include all of this copyright and permission notice ('as
  annotated') and this shall be included in all copies or substantial portions
  of the software and where the software use is visible to an end-user.
  ...
  TORT OR OTHERWISE, ARISING FROM, OR OUT OF OR IN CONNECTION WITH THE SOFTWARE
  OR THE USE OR OTHER DEALINGS IN THE SOFTWARE. See more at http://dsbird.org.uk
*/
#include <Arduino.h>
...
#endif
#define MYFS                                                                   \
  LittleFS // In preparation for the introduction of LITTLFS see
           // https://github.com/lorol/LITTLEFS replace SPIFFS with LITTLEFS
...
void Handle_File_Rename(AsyncWebServerRequest *request, String filename,
                        int Args);
void Page_Not_Found();
...
int start, downloadtime = 1, uploadtime = 1, downloadsize, uploadsize,
           downloadrate, uploadrate, numfiles;
float Temperature =
    21.34; // for example new page, amend in a snesor function if required
String Name = "Dave";
...
// #############################################################################################
void _Dir(AsyncWebServerRequest *request) {
  String Fname1, Fname2;
  int index = 0;
  Directory(); // Get a list of the current files on the MYFS
  webpage = HTML_Header();
  webpage += "<h3>Filing System Content</h3><br>";
  if (numfiles > 0) {
    webpage += "<table class='center'>";
    webpage += "<tr><th>Type</th><th>File Name</th><th>File Size</th><th "
               "class='sp'></th><th>Type</th><th>File Name</th><th>File "
               "Size</th></tr>";
    while (index < numfiles) {
      Fname1 = Filenames[index].filename;
      Fname2 = Filenames[index + 1].filename;
      webpage += "<tr>";
      webpage += "<td style = 'width:5%'>" + Filenames[index].ftype +
                 "</td><td style = 'width:25%'>" + Fname1 +
                 "</td><td style = 'width:10%'>" + Filenames[index].fsize +
                 "</td>";
      webpage += "<td class='sp'></td>";
      if (index < numfiles - 1) {
        webpage += "<td style = 'width:5%'>" + Filenames[index + 1].ftype +
                   "</td><td style = 'width:25%'>" + Fname2 +
                   "</td><td style = 'width:10%'>" +
                   Filenames[index + 1].fsize + "</td>";
      }
      webpage += "</tr>";
      index = index + 2;
    }
    webpage += "</table>";
    webpage +=
        "<p style='background-color:yellow;'><b>" + MessageLine + "</b></p>";
    MessageLine = "";
  } else {
...
void UploadFileSelect() {
  ...
  webpage += "<form method = 'POST' action = '/handleupload' "
...
void Format() {
  ...
  webpage += "<input type='radio' id='YES' name='format' value = 'YES'><label "
             "for='YES'>YES</label><br><br>";
  webpage += "<input type='radio' id='NO'  name='format' value = 'NO' "
             "checked><label for='NO'>NO</label><br><br>";
...
void handleFileUpload(AsyncWebServerRequest *request, const String &filename,
  ...
  if (!index) {
    ...
    if (!filename.startsWith("/"))
      file = "/" + filename;
    request->_tempFile = MYFS.open(file, "w");
...
// #############################################################################################
//  Not found handler is also the handler for 'delete', 'download' and 'stream'
//  functions
void notFound(AsyncWebServerRequest *request) { // Process selected file types
  ...
  if (request->url().startsWith("/downloadhandler") ||
    ...
    // Now get the filename and handle the request for 'delete' or 'download' or
    // 'stream' functions
    ...
    if (request->url().startsWith("/downloadhandler")) {
      ...
      AsyncWebServerResponse *response =
          request->beginResponse(contentType, file.size(),
                                 [file](uint8_t *buffer, size_t maxLen,
                                        size_t total) mutable -> size_t {
                                   File filecopy = file;
                                   int bytes = filecopy.read(buffer, maxLen);
                                   if (bytes + total == filecopy.size())
                                     filecopy.close();
                                   return max(0, bytes);
                                 });
    ...
    if (request->url().startsWith("/streamhandler")) {
      ...
      AsyncWebServerResponse *response =
          request->beginResponse(MYFS, filename, ContentType);
      request->send(response);
      downloadsize = GetFileSize(filename);
      downloadtime = millis() - start;
      request->redirect("/dir");
    ...
    if (request->url().startsWith("/renamehandler")) {
      Handle_File_Rename(request, filename,
                         request->args()); // Build webpage ready for display
      request->send(200, "text/html", webpage);
    }
  } else {
    Page_Not_Found();
...
void Handle_File_Download() {
  String filename = "";
  int index = 0;
  Directory(); // Get a list of files on the MYFS
  webpage = HTML_Header();
  webpage += "<h3>Select a File to Download</h3>";
  webpage += "<table>";
  webpage += "<tr><th>File Name</th><th>File Size</th></tr>";
  while (index < numfiles) {
    webpage += "<tr><td><a href='" + Filenames[index].filename + "'></a><td>" +
...
String getContentType(
    String filenametype) { // Tell the browser what file type is being sent
...
void Select_File_For_Function(String title, String function) {
  ...
  webpage += "<table class='center'>";
  webpage += "<tr><th>File Name</th><th>File Size</th><th "
             "class='sp'></th><th>File Name</th><th>File Size</th></tr>";
  while (index < numfiles) {
    Fname1 = Filenames[index].filename;
    Fname2 = Filenames[index + 1].filename;
    ...
    webpage += "<tr>";
    webpage += "<td style='width:25%'><button><a href='" + function + "~/" +
               Fname1 + "'>" + Fname1 +
               "</a></button></td><td style = 'width:10%'>" +
               Filenames[index].fsize + "</td>";
    webpage += "<td class='sp'></td>";
    if (index < numfiles - 1) {
      webpage += "<td style='width:25%'><button><a href='" + function + "~/" +
                 Fname2 + "'>" + Fname2 +
...
int GetFileSize(String filename) {
  int filesize;
  File CheckFile = MYFS.open(filename, "r");
...
void Home() {
  ...
  webpage += "<h3>File Management - Directory, Upload, Download, Stream and "
             "Delete File Examples</h3>";
  webpage += HTML_Footer();
}
// #############################################################################################
void LogOut() {
  ...
  webpage += "<h4>NOTE: On most browsers you must close all windows for this "
             "to take effect...</h4>";
  webpage += HTML_Footer();
}
// #############################################################################################
void Display_New_Page() {
  ...
  webpage += "<p>NOTE:</p>";
  webpage +=
      "<p>You can display global variables like this: " +
      String(Temperature, 2) +
      "°C</p>"; // Convert numeric variables to a string, use 2-decimal places
  webpage += "<p>Or, like this, my name is: " + Name +
             "</p>";        // no need to convert string variables
  webpage += HTML_Footer(); // Add HTML footers, don't forget to add a menu name
                            // in header!
}
// #############################################################################################
void Page_Not_Found() {
  ...
  webpage += "<p>The page you were looking for was not found, it may have been "
             "moved or is currently unavailable.</p>";
  webpage +=
      "<p>Please check the address is spelt correctly and try again.</p>";
  webpage +=
      "<p>Or click <b><a href='/'>[Here]</a></b> for the home page.</p></div>";
  webpage += HTML_Footer();
}
// #############################################################################################
void Display_System_Info() {
  ...
  if (WiFi.scanComplete() == -2)
    WiFi.scanNetworks(true,
                      false); // Scan parameters are (async, show_hidden) if
                              // async = true, don't wait for the result
  webpage = HTML_Header();
  webpage += "<h3>System Information</h3>";
  webpage += "<h4>Transfer Statistics</h4>";
  webpage += "<table class='center'>";
  webpage += "<tr><th>Last Upload</th><th>Last "
             "Download/Stream</th><th>Units</th></tr>";
  webpage += "<tr><td>" + ConvBinUnits(uploadsize, 1) + "</td><td>" +
             ConvBinUnits(downloadsize, 1) + "</td><td>File Size</td></tr> ";
  webpage += "<tr><td>" +
             ConvBinUnits((float)uploadsize / uploadtime * 1024.0, 1) +
             "/Sec</td>";
  webpage += "<td>" +
             ConvBinUnits((float)downloadsize / downloadtime * 1024.0, 1) +
             "/Sec</td><td>Transfer Rate</td></tr>";
  webpage += "</table>";
  webpage += "<h4>Filing System</h4>";
  webpage += "<table class='center'>";
  webpage += "<tr><th>Total Space</th><th>Used Space</th><th>Free "
             "Space</th><th>Number of Files</th></tr>";
#if (MYFS == ISSPIFFS)
  webpage += "<tr><td>" + ConvBinUnits(MYFS.totalBytes(), 1) + "</td>";
  webpage += "<td>" + ConvBinUnits(MYFS.usedBytes(), 1) + "</td>";
  webpage +=
      "<td>" + ConvBinUnits(MYFS.totalBytes() - MYFS.usedBytes(), 1) + "</td>";
  webpage += "<td>" +
             (numfiles == 0 ? "Pending Dir or Empty" : String(numfiles)) +
             "</td></tr>";
#endif
  webpage += "</table>";
  ...
  webpage += "<tr><td>Number of Cores</td><td>" + String(chip_info.cores) +
             "</td></tr>";
  webpage += "<tr><td>Chip revision</td><td>" + String(chip_info.revision) +
             "</td></tr>";
  webpage +=
      "<tr><td>Internal or External Flash Memory</td><td>" +
      String(((chip_info.features & CHIP_FEATURE_EMB_FLASH) ? "Embedded"
                                                            : "External")) +
      "</td></tr>";
  webpage += "<tr><td>Flash Memory Size</td><td>" +
             String((spi_flash_get_chip_size() / (1024 * 1024))) +
             " MB</td></tr>";
  webpage += "<tr><td>Current Free RAM</td><td>" +
             ConvBinUnits(ESP.getFreeHeap(), 1) + "</td></tr>";
  webpage += "</table>";
  webpage += "<h4>Network Information</h4>";
  webpage += "<table class='center'>";
  webpage += "<tr><th>Parameter</th><th>Value</th></tr>";
  webpage += "<tr><td>LAN IP Address</td><td>" +
             String(WiFi.localIP().toString()) + "</td></tr>";
  webpage += "<tr><td>Network Adapter MAC Address</td><td>" +
             String(WiFi.BSSIDstr()) + "</td></tr>";
  webpage += "<tr><td>WiFi SSID</td><td>" + String(WiFi.SSID()) + "</td></tr>";
  webpage +=
      "<tr><td>WiFi RSSI</td><td>" + String(WiFi.RSSI()) + " dB</td></tr>";
  webpage +=
      "<tr><td>WiFi Channel</td><td>" + String(WiFi.channel()) + "</td></tr>";
//   webpage += "<tr><td>WiFi Encryption Type</td><td>" +
//              String(EncryptionType(WiFi.encryptionType(0))) + "</td></tr>";
  webpage += "</table> ";
  webpage += HTML_Footer();
}
// #############################################################################################
String ConvBinUnits(size_t bytes, byte resolution) {
  if (bytes < 1024) {
    ...
    return "4711"; //FIXME return String(bytes / 1024.0, resolution) + " KB";
  } else if (bytes < (1024 * 1024 * 1024)) {
    return "4711"; //FIXME return String(bytes / 1024.0 / 1024.0, resolution) + " MB";
  } else
    return "";
}
// #############################################################################################
...
bool StartMDNSservice(const char *Name) {
  ...
  mdns_hostname_set(Name); // Set hostname ESP8266
#elif ESP8266
  printf("MDNS für ESP8266 nicht verfügbar");
  return false;
#endif
  return true;
}
...
String HTML_Header() {
  ...
  page += "<meta charset='UTF-8'>"; // Needed if you want to display special
                                    // characters like the ° symbol
  page += "<style>";
  page += "body "
          "{width:75em;margin-left:auto;margin-right:auto;font-family:Arial,"
          "Helvetica,sans-serif;font-size:16px;color:blue;background-color:#"
          "e1e1ff;text-align:center;}";
  page += "footer {padding:0.08em;background-color:cyan;font-size:1.1em;}";
  page +=
      "table "
      "{font-family:arial,sans-serif;border-collapse:collapse;width:70%;}"; // 70% of 75em!
  ...
  page += "h4 "
          "{color:slateblue;font:0.8em;text-align:left;font-style:oblique;text-"
          "align:center;}";
  page += ".center {margin-left:auto;margin-right:auto;}";
  page += ".topnav {overflow: hidden;background-color:lightcyan;}";
  page += ".topnav a {float:left;color:blue;text-align:center;padding:0.6em "
          "0.6em;text-decoration:none;font-size:1.3em;}";
  ...
  // page += "<a href='/format'>Format MYFS</a>";
...
void setup() {
  ...
  if (WiFi.scanComplete() == -2)
    WiFi.scanNetworks(true); // Complete an initial scan for WiFi networks,
...
#ifdef AccessControl
    if (!request->authenticate(http_username,
                               http_password)) // Comment out to remove need for
                                               // login username & password
      return request->requestAuthentication(); // Comment out to remove need for
                                               // login username & password
#endif
  ...
  server.on("/download", HTTP_GET, [](AsyncWebServerRequest *request) {
    ...
    Select_File_For_Function(
        "[DOWNLOAD]", "downloadhandler"); // Build webpage ready for display
    request->send(200, "text/html", webpage);
  });
  ...
  server.on("/stream", HTTP_GET, [](AsyncWebServerRequest *request) {
    ...
    Select_File_For_Function(
        "[STREAM]", "streamhandler"); // Build webpage ready for display
    request->send(200, "text/html", webpage);
  });
  ...
  server.on("/dir", HTTP_GET, [](AsyncWebServerRequest *request) {
    ...
    _Dir(request); // Build webpage ready for display
    request->send(200, "text/html", webpage);
  });
  ...
  server.on("/delete", HTTP_GET, [](AsyncWebServerRequest *request) {
    ...
    Select_File_For_Function(
        "[DELETE]", "deletehandler"); // Build webpage ready for display
    request->send(200, "text/html", webpage);
  });
  ...
  server.on("/icon", HTTP_GET, [](AsyncWebServerRequest *request) {
    request->send(MYFS, "/icon.gif", "image/gif");
  });

  // ##################### NOT FOUND HANDLER #########################
  server.onNotFound(notFound);

  // TO add new pages add a handler here, make  sure the HTML Header has a menu
  // item for it, create a page for it using a function. Copy this one, rename
  // it.
  // ##################### NEW PAGE HANDLER ##########################
...
// #############################################################################################
void Directory() {
  numfiles = 0; // Reset number of MYFS files counter
  File root = MYFS.open("/");
  if (root) {
    root.rewindDirectory();
    File file = root.openNextFile();
    while (file) { // Now get all the filenames, file types and sizes
      Filenames[numfiles].filename = (String(file.name()).startsWith("/")
...
void Handle_File_Delete(String filename) { // Delete the file
  ...
  if (!filename.startsWith("/"))
    filename = "/" + filename;
  File dataFile =
      MYFS.open(filename, "r"); // Now read MYFS to see if file exists
...
void File_Rename() { // Rename the file
  ...
  int index = 0;
  while (index < numfiles) {
    webpage += "<tr><td><input type='text' name='oldfile' style='color:blue;' "
               "value = '" +
               Filenames[index].filename + "' readonly></td>";
    webpage += "<td><input type='text' name='newfile'></td><td><input "
               "type='radio' name='choice'></tr>";
    index++;
  }
  webpage += "</table><br>";
  webpage += "<input type='submit' value='Enter'>";
...
void Handle_File_Rename(AsyncWebServerRequest *request, String filename,
  ...
  if (CurrentFile && filename != "/" && newfilename != "/" &&
      (filename != newfilename)) { // It does so rename it, ignore if no entry
                                   // made, or Newfile name exists already
    if (MYFS.rename(filename, newfilename)) {
      filename = filename.substring(1);
      newfilename = newfilename.substring(1);
      webpage += "<h3>File '" + filename + "' has been renamed to '" +
                 newfilename + "'</h3>";
      webpage += "<a href='/dir'>[Enter]</a><br><br>";
    }
  } else {
