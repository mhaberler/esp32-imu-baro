...
/*
  ...
  This example shows how to query a u-blox module for its lat/long/altitude. We
  also turn off the NMEA output on the I2C port. This decreases the amount of
  I2C traffic dramatically.

  Note: Long/lat are large numbers because they are * 10^7. To convert lat/long
  to something google maps understands simply divide the numbers by 10,000,000.
  We do this so that we don't have to use floating point numbers.

  Leave NMEA parsing behind. Now you can simply ask the module for the datums
  you want!
...
long lastTime =
    0; // Simple local timer. Limits amount if I2C traffic to u-blox module.
...
void setup() {
  ...
  // myGNSS.enableDebugging(); // Uncomment this line to enable helpful debug
  // messages on Serial
  ...
  GPSSerial.begin(9600, SERIAL_8N1, 13, 14); // GPS init
}

void loop() {
  // Query module only every second. Doing it more often will just cause I2C
  // traffic. The module only responds when a new position is available
  if (millis() - lastTime > 1000) {
    lastTime = millis(); // Update the timer

    long latitude = myGNSS.getLatitude();
    Serial.print(F("Neo Lat: "));
    ...
    if (gps.location.isValid()) {
      Serial.printf("serial lat=%f lon=%f alt=%f\n", gps.location.lat(),
