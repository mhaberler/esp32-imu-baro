...
void
reporter (config_t &config, options_t &opt)
{
#ifdef REPORT_PIN
          ...
          if (options.teleplot_viewer)
            {
              switch (bp.typus)
                ...
                case TYPE_BMP3XX:
                  ...
                  teleplot.update_ms (dev + ".hpa", bp.timestamp, bp.baro.hpa,
                                      hpascal, TELEPLOT_FLAG_NOPLOT);
                  teleplot.update_ms (dev + ".alt", bp.timestamp, bp.baro.alt,
...
static void
emitJson (const config_t &config, options_t &opt, jdoc_t &doc)
{
  if (!doc.isNull ())
    {
      if (!options.ndjson)
...
static void
emitStats (config_t &config, options_t &opt, jdoc_t &doc)
{
  if (opt.timing_stats)
    {
      if (opt.teleplot_viewer)
        {
          ...
          teleplot.update_ms ("handleImu.mean", millis (), imuStats.Mean (),
                              uS, TELEPLOT_FLAG_NOPLOT);
          teleplot.update_ms ("reporter.mean", millis (),
                              reporterStats.Mean (), uS, TELEPLOT_FLAG_NOPLOT);
          teleplot.update_ms ("customImu.mean", millis (),
                              customImuStats.Mean (), uS,
                              TELEPLOT_FLAG_NOPLOT);
  ...
  if (opt.memory_usage)
    {
      config.reporter_stack_util = (int)(100.0
                                         * (opt.reporter_stack
                                            - uxTaskGetStackHighWaterMark (
                                                reporterTask->GetHandle ()))
                                         / opt.reporter_stack);
      if (sensorTask)
        {
          config.sensor_stack_util = (int)(100.0
                                           * (opt.sensor_stack
                                              - uxTaskGetStackHighWaterMark (
                                                  sensorTask->GetHandle ()))
                                           / opt.sensor_stack);
        }
