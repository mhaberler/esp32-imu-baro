...
void
help (options_t &options, config_t &config, CmdParser *cp)
{
  Console.fmtln ("Type your commands. Supported: ");
  Console.fmtln ("  help");
  Console.fmtln ("  wire [bus sda scl khz] query or set I2C bus setting");
  Console.fmtln ("  spi [bus miso mosi sck kHz] query or set SPI bus "
  ...
  Console.fmtln ("  bs [<index> name macaddress type usage]"
  ...
  Console.fmtln ("  ndjson # toggle NDJSON printing: {}",
                 B2S (options.ndjson));
...
initShell (void)
{
        ...
        for (auto i = 0; i < NUM_I2C; i++)
          {
            ...
            Console.fmtln ("WIRE{}: sda={} scl={} kHz={}", i, ip->sda, ip->scl,
                           ip->kHz);
          }
        return;
      }
        ...
        for (auto i = 0; i < NUM_BLESENSORS; i++)
          {
            ...
            switch (bp->type)
              ...
              default:
                n++;
                Console.fmtln ("sensor {}: name={} type={} usage={} mac={}", i,
                               bp->name, (uint32_t)bp->type,
                               (uint32_t)bp->usage, bp->blemac);
              }
          }
  ...
  cmdCallback.addCmd ("PASS", [] (CmdParser *cp) {
    if (cp->getParamCount () != (options.num_ssid + 1))
      {
        Console.fmtln ("pass: same number as SSID's must be given (found {})",
                       cp->getParamCount ());
        return;
      }
  ...
  cmdCallback.addCmd ("PAR", [] (CmdParser *cp) {
    Console.fmtln ("imu={} baro={} ned={} cal={} kf={}",
                   options.selected_imu_name, sensor_types[options.which_baro],
    ...
    Console.fmtln ("BLE rate={:.1f} ads={}", options.ble_rate, config.ble_ads);
...
void
serialwriteFunc (const uint8_t writeChar)
{
  Console.write (writeChar);
}

void
serialCmdComplete (CmdParser &cmdParser, bool found)
{
  if (!found)
    {
...
void
printHelp (options_t &options, config_t &config)
{
