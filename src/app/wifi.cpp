...
bool
WifiSetup (const options_t &opt, config_t &config)
{
  time_t tt, tmp;
  int n;
  wl_status_t status;
  ...
  status = (wl_status_t)wifiMulti.run ();
  switch (status)
    {
    case WL_DISCONNECTED: // no AP, use Serial
      LOGI ("no matching WiFi access points found - using Serial for "
            "reporting");
#ifdef TELEPLOT
    ...
    case WL_CONNECTED: // all good
      ...
      LOGD ("timeClient.forceUpdate: {}", T2OK (timeClient.forceUpdate ()));
      ...
      config.millis_offset = config.timeSinceEpoch - config.timeinfo_millis;
      ...
      if (*opt.tpHost != '\0')
        {
          ...
          if (!WiFi.hostByName (opt.tpHost, teleplotSrv))
            {
              LOGE ("DNS resolve of {} failed", opt.tpHost);
              goto RUN_MDNS;
            }
          LOGD ("using manually defined teleplot destination {}:{}",
                teleplotSrv.toString ().c_str (), opt.tpPort);
          config.currentTpHost = teleplotSrv;
          config.currentTpPort = opt.tpPort;
#ifdef TELEPLOT

          teleplot.begin (teleplotSrv, opt.tpPort, config.millis_offset);
#endif
        }
      else
        {
          ...
          if (n > -1)
              ...
              for (auto i = 0; i < n; i++)
                {
                  ...
                  if ((tpHost & WiFi.subnetMask ())
                      == (WiFi.localIP () & WiFi.subnetMask ()))
                    {
                      ...
                      teleplot.begin (tpHost, tpPort, config.millis_offset);
#endif
            ...
            {
              LOGI ("tpdest not set and no result from mDNS discovery "
                    "for "
                    "teleplot/udp");
              LOGI ("using teleplot on Serial port");
#ifdef TELEPLOT
              teleplot.begin (&Serial);
#endif
            }
...
int
browseService (const char *service, const char *proto)
{
