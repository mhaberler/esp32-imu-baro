...
/*!
 *  @file Adafruit_MPU6886.cpp
 *
 *  @mainpage Adafruit MPU6886 proximity and ambient light sensor library
 *
 *  @section intro_sec Introduction
 *
 * 	I2C Driver for the MPU6886 proximity and ambient light sensor library
 *
...
/*!
 *    @brief  Instantiates a new MPU6886 class
 */
Adafruit_MPU6886::Adafruit_MPU6886(void) {}

/*!
 *    @brief  Cleans up the MPU6886 class
 */
Adafruit_MPU6886::~Adafruit_MPU6886(void) {
  if (temp_sensor)
    delete temp_sensor;
...
bool Adafruit_MPU6886::begin(uint8_t i2c_address, TwoWire *wire,
                             int32_t sensor_id) {
  if (i2c_dev) {
  ...
  // MPU6886 to come up after initial power.
  ...
  Adafruit_BusIO_Register chip_id =
      Adafruit_BusIO_Register(i2c_dev, MPU6886_WHO_AM_I, 1);

  // make sure we're talking to the right chip
  if (chip_id.read() != MPU6886_DEVICE_ID) {
    return false;
  }
 ...
 */
bool Adafruit_MPU6886::_init(int32_t sensor_id) {
  ...
  setFilterBandwidth(MPU6886_BAND_260_HZ);

  setGyroRange(MPU6886_RANGE_500_DEG);

  setAccelerometerRange(MPU6886_RANGE_2_G); // already the default

  Adafruit_BusIO_Register power_mgmt_1 =
      Adafruit_BusIO_Register(i2c_dev, MPU6886_PWR_MGMT_1, 1);
  ...
  temp_sensor = new Adafruit_MPU6886_Temp(this);
  accel_sensor = new Adafruit_MPU6886_Accelerometer(this);
  gyro_sensor = new Adafruit_MPU6886_Gyro(this);
...
*/
/**************************************************************************/
void Adafruit_MPU6886::reset(void) {
  Adafruit_BusIO_Register power_mgmt_1 =
      Adafruit_BusIO_Register(i2c_dev, MPU6886_PWR_MGMT_1, 1);
  Adafruit_BusIO_Register sig_path_reset =
      Adafruit_BusIO_Register(i2c_dev, MPU6886_SIGNAL_PATH_RESET, 1);
...
*/
/**************************************************************************/
uint8_t Adafruit_MPU6886::getSampleRateDivisor(void) {
  Adafruit_BusIO_Register sample_rate_div =
      Adafruit_BusIO_Register(i2c_dev, MPU6886_SMPLRT_DIV, 1);
...
*/
/**************************************************************************/
void Adafruit_MPU6886::setSampleRateDivisor(uint8_t divisor) {
  Adafruit_BusIO_Register sample_rate_div =
      Adafruit_BusIO_Register(i2c_dev, MPU6886_SMPLRT_DIV, 1);
...
*/
/**************************************************************************/
mpu6886_accel_range_t Adafruit_MPU6886::getAccelerometerRange(void) {
  Adafruit_BusIO_Register accel_config =
      Adafruit_BusIO_Register(i2c_dev, MPU6886_ACCEL_CONFIG, 1);
  Adafruit_BusIO_RegisterBits accel_range =
      Adafruit_BusIO_RegisterBits(&accel_config, 2, 3);

  return (mpu6886_accel_range_t)accel_range.read();
}
...
/*!
    ...
    @param  new_range
            The new range to set. Must be a `mpu6886_accel_range_t`
*/
/**************************************************************************/
void Adafruit_MPU6886::setAccelerometerRange(mpu6886_accel_range_t new_range) {
  Adafruit_BusIO_Register accel_config =
      Adafruit_BusIO_Register(i2c_dev, MPU6886_ACCEL_CONFIG, 1);
...
/*!
    ...
    @return  The `mpu6886_gyro_range_t` gyroscope measurement range
*/
/**************************************************************************/
mpu6886_gyro_range_t Adafruit_MPU6886::getGyroRange(void) {
  Adafruit_BusIO_Register gyro_config =
      Adafruit_BusIO_Register(i2c_dev, MPU6886_GYRO_CONFIG, 1);
  Adafruit_BusIO_RegisterBits gyro_range =
      Adafruit_BusIO_RegisterBits(&gyro_config, 2, 3);

  return (mpu6886_gyro_range_t)gyro_range.read();
}
...
/*!
    ...
    @param  new_range
            The new range to set. Must be a `mpu6886_gyro_range_t`
*/
/**************************************************************************/
void Adafruit_MPU6886::setGyroRange(mpu6886_gyro_range_t new_range) {
  Adafruit_BusIO_Register gyro_config =
      Adafruit_BusIO_Register(i2c_dev, MPU6886_GYRO_CONFIG, 1);
  Adafruit_BusIO_RegisterBits gyro_range =
      Adafruit_BusIO_RegisterBits(&gyro_config, 2, 3);
...
/*!
    ...
    @param  new_clock
            The clock source to set. Must be a `mpu6886_clock_select_t`
*/
/**************************************************************************/
void Adafruit_MPU6886::setClock(mpu6886_clock_select_t new_clock) {
...
/*!
    ...
    @return  The current `mpu6886_clock_select_t` clock source
*/
/**************************************************************************/
mpu6886_clock_select_t Adafruit_MPU6886::getClock(void) {
  Adafruit_BusIO_Register pwr_mgmt =
      Adafruit_BusIO_Register(i2c_dev, MPU6886_PWR_MGMT_1, 1);

  Adafruit_BusIO_RegisterBits clock_select =
      Adafruit_BusIO_RegisterBits(&pwr_mgmt, 3, 0);
  return (mpu6886_clock_select_t)clock_select.read();
}
 ...
 */
/**************************************************************************/
mpu6886_fsync_out_t Adafruit_MPU6886::getFsyncSampleOutput(void) {
  Adafruit_BusIO_Register config =
      Adafruit_BusIO_Register(i2c_dev, MPU6886_CONFIG, 1);
  Adafruit_BusIO_RegisterBits fsync_out =
      Adafruit_BusIO_RegisterBits(&config, 3, 3);
  return (mpu6886_fsync_out_t)fsync_out.read();
}
...
*     @brief  Sets the location that the FSYNC pin sample is stored
*     @param  fsync_output
              a `mpu6886_fsync_out_t` to specify the LSB of which data register
              should be used to store the state of the FSYNC pin
*/
/**************************************************************************/
void Adafruit_MPU6886::setFsyncSampleOutput(mpu6886_fsync_out_t fsync_output) {
  Adafruit_BusIO_Register config =
      Adafruit_BusIO_Register(i2c_dev, MPU6886_CONFIG, 1);
  Adafruit_BusIO_RegisterBits fsync_out =
      Adafruit_BusIO_RegisterBits(&config, 3, 3);
 ...
 *     @return  The current `mpu6886_bandwidth_t` filter bandwidth
 */
/**************************************************************************/
mpu6886_bandwidth_t Adafruit_MPU6886::getFilterBandwidth(void) {
  Adafruit_BusIO_Register config =
      Adafruit_BusIO_Register(i2c_dev, MPU6886_CONFIG, 1);

  Adafruit_BusIO_RegisterBits filter_config =
      Adafruit_BusIO_RegisterBits(&config, 3, 0);
  return (mpu6886_bandwidth_t)filter_config.read();
}

/**************************************************************************/
/*!
 *    @brief Sets the bandwidth of the Digital Low-Pass Filter
 *    @param bandwidth the new `mpu6886_bandwidth_t` bandwidth
 */
/**************************************************************************/
void Adafruit_MPU6886::setFilterBandwidth(mpu6886_bandwidth_t bandwidth) {
  Adafruit_BusIO_Register config =
      Adafruit_BusIO_Register(i2c_dev, MPU6886_CONFIG, 1);
 ...
 *     @return  The current `mpu6886_highpass_t` filter bandwidth
 */
/**************************************************************************/
mpu6886_highpass_t Adafruit_MPU6886::getHighPassFilter(void) {
  Adafruit_BusIO_Register config =
      Adafruit_BusIO_Register(i2c_dev, MPU6886_ACCEL_CONFIG, 1);

  Adafruit_BusIO_RegisterBits filter_config =
      Adafruit_BusIO_RegisterBits(&config, 3, 0);
  return (mpu6886_highpass_t)filter_config.read();
}

/**************************************************************************/
/*!
 *    @brief Sets the bandwidth of the Digital High-Pass Filter
 *    @param bandwidth the new `mpu6886_highpass_t` bandwidth
 */
/**************************************************************************/
void Adafruit_MPU6886::setHighPassFilter(mpu6886_highpass_t bandwidth) {
  Adafruit_BusIO_Register config =
      Adafruit_BusIO_Register(i2c_dev, MPU6886_ACCEL_CONFIG, 1);
...
*/
/**************************************************************************/
void Adafruit_MPU6886::setInterruptPinPolarity(bool active_low) {
  Adafruit_BusIO_Register int_pin_config =
      Adafruit_BusIO_Register(i2c_dev, MPU6886_INT_PIN_CONFIG, 1);
...
*/
/**************************************************************************/
void Adafruit_MPU6886::setInterruptPinLatch(bool held) {
  Adafruit_BusIO_Register int_pin_config =
      Adafruit_BusIO_Register(i2c_dev, MPU6886_INT_PIN_CONFIG, 1);
...
*/
/**************************************************************************/
void Adafruit_MPU6886::setMotionInterrupt(bool active) {
  Adafruit_BusIO_Register int_enable =
      Adafruit_BusIO_Register(i2c_dev, MPU6886_INT_ENABLE, 1);
 ...
 */
/**************************************************************************/
bool Adafruit_MPU6886::getMotionInterruptStatus(void) {
  Adafruit_BusIO_Register status =
      Adafruit_BusIO_Register(i2c_dev, MPU6886_INT_STATUS, 1);
 ...
 */
/**************************************************************************/
void Adafruit_MPU6886::setMotionDetectionThreshold(uint8_t thr) {
  Adafruit_BusIO_Register threshold =
      Adafruit_BusIO_Register(i2c_dev, MPU6886_MOT_THR, 1);
  threshold.write(thr);
}
 ...
 */
/**************************************************************************/
void Adafruit_MPU6886::setMotionDetectionDuration(uint8_t dur) {
  Adafruit_BusIO_Register duration =
      Adafruit_BusIO_Register(i2c_dev, MPU6886_MOT_DUR, 1);
  duration.write(dur);
}
...
*/
/**************************************************************************/
void Adafruit_MPU6886::setI2CBypass(bool bypass) {
  Adafruit_BusIO_Register int_pin_config =
      Adafruit_BusIO_Register(i2c_dev, MPU6886_INT_PIN_CONFIG, 1);
  ...
  Adafruit_BusIO_Register user_ctrl =
      Adafruit_BusIO_Register(i2c_dev, MPU6886_USER_CTRL, 1);
...
*/
/**************************************************************************/
bool Adafruit_MPU6886::enableSleep(bool enable) {
  Adafruit_BusIO_Register pwr_mgmt =
      Adafruit_BusIO_Register(i2c_dev, MPU6886_PWR_MGMT_1, 1);
...
*/
/**************************************************************************/
bool Adafruit_MPU6886::enableCycle(bool enable) {
  Adafruit_BusIO_Register pwr_mgmt =
      Adafruit_BusIO_Register(i2c_dev, MPU6886_PWR_MGMT_1, 1);
 ...
 */
/**************************************************************************/
mpu6886_cycle_rate_t Adafruit_MPU6886::getCycleRate(void) {
  Adafruit_BusIO_Register pwr_mgmt_2 =
      Adafruit_BusIO_Register(i2c_dev, MPU6886_PWR_MGMT_2, 1);

  Adafruit_BusIO_RegisterBits cycle_rate =
      Adafruit_BusIO_RegisterBits(&pwr_mgmt_2, 2, 6);
  return (mpu6886_cycle_rate_t)cycle_rate.read();
}
 ...
 *     @param  rate
 *              The `mpu6886_cycle_rate_t` specifying the desired
 *              measurement rate
 */
/**************************************************************************/
void Adafruit_MPU6886::setCycleRate(mpu6886_cycle_rate_t rate) {
  Adafruit_BusIO_Register pwr_mgmt_2 =
      Adafruit_BusIO_Register(i2c_dev, MPU6886_PWR_MGMT_2, 1);
 ...
 */
/**************************************************************************/
bool Adafruit_MPU6886::setGyroStandby(bool xAxisStandby, bool yAxisStandby,
                                      bool zAxisStandby) {
  Adafruit_BusIO_Register pwr_mgmt_2 =
      Adafruit_BusIO_Register(i2c_dev, MPU6886_PWR_MGMT_2, 1);
 ...
 */
/**************************************************************************/
bool Adafruit_MPU6886::setAccelerometerStandby(bool xAxisStandby,
                                               bool yAxisStandby,
                                               bool zAxisStandby) {
  Adafruit_BusIO_Register pwr_mgmt_2 =
      Adafruit_BusIO_Register(i2c_dev, MPU6886_PWR_MGMT_2, 1);
 ...
 */
/**************************************************************************/
bool Adafruit_MPU6886::setTemperatureStandby(bool enable) {
  Adafruit_BusIO_Register pwr_mgmt =
      Adafruit_BusIO_Register(i2c_dev, MPU6886_PWR_MGMT_1, 1);
 ...
 */
/**************************************************************************/
void Adafruit_MPU6886::_read(void) {
  // get raw readings
  Adafruit_BusIO_Register data_reg =
      Adafruit_BusIO_Register(i2c_dev, MPU6886_ACCEL_OUT, 14);
  ...
  mpu6886_accel_range_t accel_range = getAccelerometerRange();

  float accel_scale = 1;
  if (accel_range == MPU6886_RANGE_16_G)
    accel_scale = 2048;
  if (accel_range == MPU6886_RANGE_8_G)
    accel_scale = 4096;
  if (accel_range == MPU6886_RANGE_4_G)
    accel_scale = 8192;
  if (accel_range == MPU6886_RANGE_2_G)
    accel_scale = 16384;
  ...
  mpu6886_gyro_range_t gyro_range = getGyroRange();

  float gyro_scale = 1;
  if (gyro_range == MPU6886_RANGE_250_DEG)
    gyro_scale = 131;
  if (gyro_range == MPU6886_RANGE_500_DEG)
    gyro_scale = 65.5;
  if (gyro_range == MPU6886_RANGE_1000_DEG)
    gyro_scale = 32.8;
  if (gyro_range == MPU6886_RANGE_2000_DEG)
    gyro_scale = 16.4;
...
*/
/**************************************************************************/
bool Adafruit_MPU6886::getEvent(sensors_event_t *accel, sensors_event_t *gyro,
...
void Adafruit_MPU6886::fillTempEvent(sensors_event_t *temp,
                                     uint32_t timestamp) {
...
void Adafruit_MPU6886::fillAccelEvent(sensors_event_t *accel,
                                      uint32_t timestamp) {
...
void Adafruit_MPU6886::fillGyroEvent(sensors_event_t *gyro,
                                     uint32_t timestamp) {
...
*/
Adafruit_Sensor *Adafruit_MPU6886::getTemperatureSensor(void) {
  return temp_sensor;
}
 ...
 */
Adafruit_Sensor *Adafruit_MPU6886::getAccelerometerSensor(void) {
  return accel_sensor;
}
 ...
 */
Adafruit_Sensor *Adafruit_MPU6886::getGyroSensor(void) { return gyro_sensor; }

/**************************************************************************/
/*!
    @brief  Gets the sensor_t data for the MPU6886's gyroscope sensor
*/
/**************************************************************************/
void Adafruit_MPU6886_Gyro::getSensor(sensor_t *sensor) {
  /* Clear the sensor_t object */
  memset(sensor, 0, sizeof(sensor_t));

  /* Insert the sensor name in the fixed length char array */
  strncpy(sensor->name, "MPU6886_G", sizeof(sensor->name) - 1);
...
*/
/**************************************************************************/
bool Adafruit_MPU6886_Gyro::getEvent(sensors_event_t *event) {
  _theMPU6886->_read();
  _theMPU6886->fillGyroEvent(event, millis());
...
/**************************************************************************/
/*!
    @brief  Gets the sensor_t data for the MPU6886's accelerometer
*/
/**************************************************************************/
void Adafruit_MPU6886_Accelerometer::getSensor(sensor_t *sensor) {
  /* Clear the sensor_t object */
  memset(sensor, 0, sizeof(sensor_t));

  /* Insert the sensor name in the fixed length char array */
  strncpy(sensor->name, "MPU6886_A", sizeof(sensor->name) - 1);
  sensor->name[sizeof(sensor->name) - 1] = 0;
  sensor->version = 1;
  sensor->sensor_id = _sensorID;
...
*/
/**************************************************************************/
bool Adafruit_MPU6886_Accelerometer::getEvent(sensors_event_t *event) {
  _theMPU6886->_read();
  _theMPU6886->fillAccelEvent(event, millis());
...
/**************************************************************************/
/*!
    @brief  Gets the sensor_t data for the MPU6886's tenperature
*/
/**************************************************************************/
void Adafruit_MPU6886_Temp::getSensor(sensor_t *sensor) {
  /* Clear the sensor_t object */
  memset(sensor, 0, sizeof(sensor_t));

  /* Insert the sensor name in the fixed length char array */
  strncpy(sensor->name, "MPU6886_T", sizeof(sensor->name) - 1);
  sensor->name[sizeof(sensor->name) - 1] = 0;
  sensor->version = 1;
  sensor->sensor_id = _sensorID;
...
*/
/**************************************************************************/
bool Adafruit_MPU6886_Temp::getEvent(sensors_event_t *event) {
  _theMPU6886->_read();
  _theMPU6886->fillTempEvent(event, millis());
